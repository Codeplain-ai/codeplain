...
# Bytes that end a plain run. pyte drops the rest of the feed() a CAN or SUB arrives in,
# so each is framed on its own.
_GROUND_BOUNDARIES = bytes((_ESC, _CAN, _SUB))


def _first_boundary(data: bytes, index: int) -> int:
    """Index of the next byte that ends a plain run, or -1 if the run reaches the end."""
    positions = [pos for pos in (data.find(b, index) for b in _GROUND_BOUNDARIES) if pos >= 0]
    ...
    def frame(self, data: bytes) -> List[bytes]:
        ...
        while index < length:
            if self._state == _GROUND:
                start = _first_boundary(data, index)
                if start < 0:
                ...
                if data[start] != _ESC:
                    units.append(data[start : start + 1])
                    index = start + 1
                    continue
                self._state = _ESCAPE
                self._pending += b"\x1b"
