...
_declare("ResumeThread", [HANDLE], DWORD)
...
def _create_process(
    ...
    ok = kernel32.CreateProcessW(
        ...
        None,
        False,
        # Suspended, so a cancellation observed by the checks below still precedes the
...
def _resume_process(proc: _ProcInfo) -> None:
    """Releases the target, once every spawn check has passed and while its handle is held."""
    if kernel32.ResumeThread(proc.pi.hThread) == RESUME_FAILED:
        raise _win_error("Starting the script", ctypes.get_last_error())
...
class ConPtyProcess(TerminalProcess):
    ...
    def _start_session(
        ...
        try:
            ...
            # slowest step here — so the check runs again on the other side of it. The
            # target is suspended until below, so an unwind here still precedes its first
            # instruction.
            self._check_spawn_interrupted()
