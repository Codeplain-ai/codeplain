...
**Memory Management** (`memory_management/`): Records what was already tried during a render so later steps get that context. Each memory is an objective record of an observed test failure, the intervention applied against it, and the outcome that followed — see `memory_management/record.py` for the schema and `memory_management/fingerprint.py` for the failure normalization that makes records addressable. Retrieval (`memory_management/retrieval.py`) works along two axes: every attempt already made against the functionality being fixed (returned in full, as a chronology) plus ranked associative evidence from elsewhere in the render (budgeted by how stuck the fix loop is). Records are JSON on disk but reach the prompt as a single rendered block (`memory_management/rendering.py`) — memory sits after the prompt-cache breakpoint, so its tokens are paid on every call.
