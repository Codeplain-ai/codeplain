...
def test_the_target_is_created_suspended_and_resumed_after_the_checks(monkeypatch, backend, tmp_path):
    """Asserted on the call the backend makes, and the script still has to exit, which is
    what proves the resume."""
    script = write_program(tmp_path, "exit_zero", "import sys\nsys.exit(0)\n")
    recorded = {}
    create = kernel32.CreateProcessW

    def recording(*arguments):
        recorded["flags"] = arguments[5]
        return create(*arguments)

    monkeypatch.setattr(kernel32, "CreateProcessW", recording)

    backend.spawn(command(script))

    assert recorded["flags"] & _conpty.CREATE_SUSPENDED
    assert wait_for_exit(backend) == 0  # resumed, or this never exits
