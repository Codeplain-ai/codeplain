...
class TestRenderOutcome:
    ...
    def test_an_interrupted_render_is_cancelled(self):
        """Ctrl-C is the user stopping the render. It does not set the cancelled
        flag, so the outcome has to be told about it."""
        assert plain2code._render_outcome(make_run_state(), interrupted=True) == "cancelled"
...
class TestMainReportsEveryEnding:
    ...
    def test_a_keyboard_interrupt_is_reported_as_cancelled(self):
        """Ctrl-C never enters a terminal state in the state machine, so this
        ending exists only in main(). In headless mode it is the only way to stop
        a render, and stopping is a cancellation, not a failure."""
    ...
    def test_a_keyboard_interrupt_leaves_the_exit_summary_alone(self):
        """The summary reads render_cancelled. Only the reported outcome changes."""
        captured = {}
        ...
        assert captured["run_state"].render_cancelled is False
