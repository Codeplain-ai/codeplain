...
def test_specification_text_is_kept_off_the_stdout_sink(configured_handlers, quiet_console, capsys):
    """Specification contents must not be exposed in logs, and stdout under CI is often a
    build log."""
    configured_handlers(headless=True)

    console.info("Rendering functionality 3: the user's whole functional requirement", spec_content=True)
    console.info("Running unit tests.")

    printed = capsys.readouterr().out
    assert "whole functional requirement" not in printed
