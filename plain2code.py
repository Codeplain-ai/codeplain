...
def _render_outcome(run_state: RunState, interrupted: bool = False) -> str:
    """Name the terminal state a render reached.

    Ctrl-C is the user stopping the render, so it is a cancellation. It does not
    set render_cancelled: the exit summary reads that flag, and its wording for
    an interrupt stays as it is.
    ...
    """
    if interrupted or run_state.render_cancelled:
...
def report_render_finished(
    codeplainAPI: codeplain_api.CodeplainAPI, run_state: RunState, interrupted: bool = False
) -> None:
    ...
    try:
        codeplainAPI.render_finished(_render_outcome(run_state, interrupted), run_state)
    except Exception:
...
def main():  # noqa: C901
    ...
    try:
        ...
        report_render_finished(codeplainAPI, run_state, interrupted)
