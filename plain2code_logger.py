...
class SpecContentFilter(logging.Filter):
    """Drops records carrying the user's specification text.

    For the headless stdout sink only. Under CI that is often a build log, and
    specification contents do not belong in one.
    """

    def filter(self, record: logging.LogRecord) -> bool:
        return not getattr(record, "spec_content", False)
